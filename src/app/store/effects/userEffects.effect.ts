...
export class UserEffects{
    ...
    constructor(private actions$: Actions, private api: AuthService, private router: Router){}
   ...
   registerUser$ = createEffect(() => this.actions$.pipe(
       ...
       switchMap(({userData}) => {
        ...
        return from(this.api.register(userData.email, userData.password)).pipe(
            map((userData) => {console.log(userData); return  RegisterUserActionSuccess({userData})}
              ))
       }),
